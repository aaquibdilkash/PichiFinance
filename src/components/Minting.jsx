...
const Minting = ({ wallet, chain, stats, reload, woolBalance }) => {
  ...
  const ethCost = () => {
    return (
      formatUnits(parseUnits(MINT_PRICE, "gwei") * BigInt(amount), "gwei") +
      " AVAX"
    );
  };
  ...
  const tractorCost = () => {
    return (
      formatUnits(parseUnits(TRACTOR_PRICE, "gwei") * BigInt(amount), "gwei") +
      " TRACTOR"
    );
  };
  ...
  const joeCost = () => {
    return (
      formatUnits(parseUnits(JOE_PRICE, "ether") * BigInt(amount), "ether") +
      " JOE"
    );
  };
  ...
  const preCheck = () => {
    ...
    if (parseUnits("" + totalWoolCost(), "gwei").gt(woolBalance))
      return "Insufficient $aWOOL";
    return undefined;
  };
          ...
          <EthereumInteraction wallet={wallet} chain={chain}>
            ...
            <div className="mb-2">
              ...
              <span className="text-red">
                {requiresEth() ? ethCost() : `${totalWoolCost()} $aWOOL`}
              </span>
              <br />
              {requiresEth() && (
                <span>
