import { Contract, BigNumber, parseUnits, Interface } from "ethers";
...
export const mint = async (stake, tokens, paid) => {
  ...
  const ethCost = parseUnits(MINT_PRICE, "ether") * BigInt(tokens);

  const gasEstimate = await contract.mint.estimateGas(tokens, {
    value: paid ? ethCost : BigInt(0),
  });
...
export const mintWithJoe = async (tokens) => {
  ...
  const contract = new Contract(process.env.REACT_APP_WOOLF, WOOLF_ABI, signer);
    const gasEstimate = await contract.mintWithJoeToken.estimateGas(tokens);
//   const gasEstimate = BigInt(10000000);
...
export const mintWithTractor = async (tokens) => {
  ...
  const signer = provider.getSigner();
  const contract = new Contract(process.env.REACT_APP_WOOLF, WOOLF_ABI, signer);
    const gasEstimate = await contract.mintWithTractorToken.estimateGas(tokens);
//   const gasEstimate = BigInt(10000000);
